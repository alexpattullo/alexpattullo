<h2 align="left">Alex</h2>

### About

Computer Science (Cyber Security) student.  
Focused on backend systems, automation, and secure software for online communities.

### Interests
- Cyber security & vulnerability analysis  
- Backend development & API design  
- Server automation & infrastructure  
- Abuse detection & prevention systems 

<p align="center">
  <a href="mailto:alex@alexpattullo.dev">
    <img src="https://img.shields.io/badge/Email-alex%40alexpattullo.dev-blue?style=flat-square&logo=gmail&logoColor=white" />
  </a>
</p>
